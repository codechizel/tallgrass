...
Implemented district-based disambiguation in both Phase 14 and Phase 14b:
...
- **DIME (Phase 14b):** New `_parse_dime_district()` parses DIME's variable-format district strings (`"KS-113"`, `"KS01"`, `"KS-7"`, `"27"`) to Int64.
...
- Phase 14 and Phase 14b callers updated to pass `start_year` — minimal code change.
...
| File | Change |
|------|--------|
| `analysis/14_external_validation/external_validation_data.py` | `_extract_sm_district()`, `_deduplicate_with_district()`, updated `_phase2_last_name_match()` and `match_legislators()` |
| `analysis/14b_external_validation_dime/external_validation_dime_data.py` | `_parse_dime_district()`, `_deduplicate_with_district()`, updated `_phase2_last_name_match()` |
| `analysis/14_external_validation/external_validation.py` | Pass `start_year` to `match_legislators()` |
