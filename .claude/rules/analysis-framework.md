...
- **MCMC (all models)**: nutpie Rust NUTS sampler — single process, Rust threads for parallel chains (ADR-0051, ADR-0053). Graph-building functions (`build_per_chamber_graph()`, `build_joint_graph()`, `build_irt_graph()`) return PyMC models without sampling. Sampling functions compile with `nutpie.compile_pymc_model()` and sample with `nutpie.sample()`. Initialization via `analysis/init_strategy.py`: `--init-strategy {auto,irt-informed,pca-informed,2d-dim1}` (ADR-0107). Auto prefers 1D IRT, falls back to PCA. `2d-dim1` uses 2D IRT Dim 1 for iterative refinement (Phase 05 only). `jitter_rvs` excludes the initialized variable. Robustness: `--dim1-prior` uses 2D IRT Dim 1 as informative prior (`xi ~ Normal(dim1, sigma)`) to recover ideology in horseshoe-affected chambers (ADR-0108).
